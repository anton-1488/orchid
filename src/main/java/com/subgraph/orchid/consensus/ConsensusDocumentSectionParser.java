...
public abstract class ConsensusDocumentSectionParser {
    ...
    public ConsensusDocumentSectionParser(DocumentFieldParser parser, ConsensusDocumentImpl document) {
        this.fieldParser = parser;
        this.document = document;
    }

    public DocumentSection parseKeywordLine() {
        String keywordString = fieldParser.getCurrentKeyword();
        if (getNextStateKeyword() != null && getNextStateKeyword().equals(keywordString)) {
            return nextSection();
        }

        DocumentKeyword keyword = DocumentKeyword.findKeyword(keywordString, getSection());
        /*
         * dirspec.txt (1.2)
